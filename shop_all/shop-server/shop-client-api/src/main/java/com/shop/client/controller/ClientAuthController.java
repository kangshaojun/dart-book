...
import com.shop.client.service.CaptchaCodeManager;
import com.shop.client.service.UserTokenManager;
...
public class ClientAuthController {
    private final Log logger = LogFactory.getLog(ClientAuthController.class);
    ...
    public Object login(@RequestBody String body, HttpServletRequest request) {
        ...
        if (userList.size() > 1) {
            ...
            return ResponseUtil.fail(ClientResponseCode.AUTH_INVALID_ACCOUNT, "账号不存在");
        } else {
            user = userList.get(0);
        }
        ...
        if (!encoder.matches(password, user.getPassword())) {
            return ResponseUtil.fail(ClientResponseCode.AUTH_INVALID_ACCOUNT, "账号密码不对");
        }
    ...
    public Object registerCaptcha(@RequestBody String body) {
        ...
        if (!notifyService.isSmsEnable()) {
            return ResponseUtil.fail(ClientResponseCode.AUTH_CAPTCHA_UNSUPPORT, "小程序后台验证码服务不支持");
        }
        ...
        if (!successful) {
            return ResponseUtil.fail(ClientResponseCode.AUTH_CAPTCHA_FREQUENCY, "验证码未超时1分钟，不能发送");
        }
    ...
    public Object register(@RequestBody String body, HttpServletRequest request) {
        ...
        List<ShopUser> userList = userService.queryByUsername(username);
        if (userList.size() > 0) {
            return ResponseUtil.fail(ClientResponseCode.AUTH_NAME_REGISTERED, "用户名已注册");
        }

        userList = userService.queryByMobile(mobile);
        if (userList.size() > 0) {
            return ResponseUtil.fail(ClientResponseCode.AUTH_MOBILE_REGISTERED, "手机号已注册");
        }
        if (!RegexUtil.isMobileExact(mobile)) {
            return ResponseUtil.fail(ClientResponseCode.AUTH_INVALID_MOBILE, "手机号格式不正确");
        }
    ...
    public Object reset(@RequestBody String body, HttpServletRequest request) {
        ...
        if (userList.size() > 1) {
            ...
            return ResponseUtil.fail(ClientResponseCode.AUTH_MOBILE_UNREGISTERED, "手机号未注册");
        } else {
            user = userList.get(0);
        }
